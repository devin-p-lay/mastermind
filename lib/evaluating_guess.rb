...
class EvaluatingGuess
  attr_reader :colors,
              :player_guess_attempt
              
  def initialize(game, secret_code)
    @user_guess = game.player_guess_attempt
  ...
  def guess_color
    ...
    puts "You have #{hint[:correct_colors]} colors correct"
  end
  ...
  def guess_position
    ...
    puts " and #{hint[:correct_positions]} colors in the correct position."
  end
end
